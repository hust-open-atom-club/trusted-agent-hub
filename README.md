...
`SKILL.md`、提示词和说明文本中的提示注入、凭据外传、破坏命令等静态命中先作为语义候选，不直接按高危计分。系统默认进行两次独立 LLM 上下文复核；结论冲突时增加第三次仲裁。只有确认是真实漏洞且具备实际破坏力时才恢复为高危；良性示例从自动评分中移除，不确定或 LLM 不可用时转人工确认而不是自动判恶意。
