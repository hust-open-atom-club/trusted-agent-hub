...
扫描结果区分“安全发现”和“审核提示”。元数据缺失只提示；缺少签名、供应链证明、SBOM 各扣 2 分；文档描述能力但未声明权限扣 5 分。这些提示均不直接改变安全等级。若结构化代码证据确认实际能力未声明，则显示高危提示并最多降一级，但不会仅凭这一项判为 E。
...
3 层 9 步漏斗模型，输出 0–100 审核分 + A–E 安全等级。两者相关但不再使用简单分数阈值强绑定：固定审核扣分可以降低分数而不改变安全等级，避免将元数据或证明材料缺失误当成恶意行为。
