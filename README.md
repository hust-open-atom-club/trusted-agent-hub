...
├── scanners/
│   └── risk_scanner/     安全扫描器 — 20 条规则 + LLM 语义复核
├── packages/
│   ├── schema/           统一数据契约 (JSON Schema) + 元数据提取器
│   └── trust-score/      独立的安全风险与证据完整度评估
...
`SKILL.md`、提示词、说明文本和上下文相关代码中的提示注入、凭据外传、破坏命令等静态命中会标记为语义候选，同时保留原始静态严重性。系统默认进行两次独立 LLM 上下文复核；结论冲突时增加第三次仲裁。只有在上下文完整、引用有效且高置信度的良性共识下，候选的有效严重性才会降为 `info`；已确认漏洞禁止降级。不确定、上下文不足或 LLM 不可用时保留复核前严重性并转人工确认。

LLM 提示词、上下文覆盖、模型信息和受控降级条件见 [LLM security adjudication](docs/llm-adjudication.md)。

### 信任评分引擎

评分输出拆成两个独立指标：`security_assessment` 给出 0–100 安全分、A–E 安全等级和结论状态；`evidence_assessment` 给出证据质量分、覆盖率、验证器状态和作者信誉。顶层兼容字段 `score` 与 `risk_summary.grade` 均镜像安全指标，来源不透明、作者是新人或验证器尚不存在都不会单独降低安全等级。

| Grade | 安全含义 | 安装建议 |
|-------|----------|----------|
| A | 可信 | 自动安装 |
| B | 低风险 | 展示权限声明后安装 |
| C | 中风险或存在待确认语义结论 | 展示扫描摘要 + 确认 |
| D | 高风险 | 强烈不建议，双重确认 |
| E | 已确认恶意、人工驳回或命中其他否决条件 | 禁止安装 |

原有 9 个明细维度继续保留用于解释：权限最小化与扫描结果驱动安全分；来源可信度、签名可验证性、元数据完整性、版本稳定性、作者信誉和用户反馈进入独立证据指标；人工安全复核可以确认、否决或要求继续复核。详细契约见 [`docs/trust-assessment-model.md`](docs/trust-assessment-model.md)。
...
支持 Claude Code / Cursor / Codex / VS Code 等多种客户端，安装前展示权限声明和信任评分。
