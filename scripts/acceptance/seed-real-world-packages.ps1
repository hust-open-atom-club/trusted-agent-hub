...
$copyPackages = @(
    ...
    # mcp-server-time 2026.7.10 与 mcp>=1.23 不兼容（McpError 改名），
    # 固定 mcp==1.9.0 + mcp-server-time==2025.9.25 可正常运行。
    @{ Name = "time-mcp";         Source = Join-Path $serversClone "src\time";              Repo = "https://github.com/modelcontextprotocol/servers/tree/main/src/time";              Type = "mcp_server"; Grade = "B"; Client = "claude-code";        Desc = "Official MCP reference server: time and timezone utilities."; Compat = @("claude-code", "cursor"); McpServers = @([pscustomobject]@{ name = "time"; command = "uvx"; args = @("--with", "mcp==1.9.0", "mcp-server-time==2025.9.25"); env = $null }); Targets = @(@{ client = "claude-code"; destination = "~/.claude/skills/time-mcp/" }, @{ client = "cursor"; destination = "~/.cursor/skills/time-mcp/" }) },
    ...
    @{ Name = "anthropic-skills-plugin"; Source = Join-Path $RepoRoot "examples\real-world\plugins\anthropic-skills-plugin"; Repo = "https://github.com/anthropics/skills"; Type = "plugin"; Grade = "A"; Client = "claude-code-plugin"; Desc = "Claude Code plugin bundling Apache-2.0 Anthropic skills (skill-creator/mcp-builder/algorithmic-art)."; Compat = @("claude-code-plugin"); Targets = @(@{ client = "claude-code-plugin"; destination = "~/.claude/skills/anthropic-skills-plugin/" }); CommitHash = "b29e7cf65e5cb78a5ac33d582270551bc74a14eb"; License = "Apache-2.0"; Homepage = "https://github.com/anthropics/skills" },
    @{ Name = "anthropic-web-skills-plugin"; Source = Join-Path $RepoRoot "examples\real-world\plugins\anthropic-web-skills-plugin"; Repo = "https://github.com/anthropics/skills"; Type = "plugin"; Grade = "B"; Client = "claude-code-plugin"; Desc = "Claude Code plugin bundling Anthropic web skills (brand/webapp-testing/theme/frontend)."; Compat = @("claude-code-plugin"); Targets = @(@{ client = "claude-code-plugin"; destination = "~/.claude/skills/anthropic-web-skills-plugin/" }); CommitHash = "b29e7cf65e5cb78a5ac33d582270551bc74a14eb"; License = "Apache-2.0"; Homepage = "https://github.com/anthropics/skills" },
    @{ Name = "superpowers"; Source = Join-Path $RepoRoot "examples\real-world\plugins\superpowers"; Repo = "https://github.com/obra/superpowers"; Type = "plugin"; Grade = "B"; Client = "claude-code-plugin"; Desc = "Superpowers: MIT core skills library for Claude Code (TDD, debugging, collaboration)."; Version = "6.2.0"; Compat = @("claude-code-plugin"); Targets = @(@{ client = "claude-code-plugin"; destination = "~/.claude/skills/superpowers/" }); CommitHash = "44c9b2d6e889982ac18c27d05a19fefe335194e1"; License = "MIT"; Homepage = "https://github.com/obra/superpowers" },
...
$seeds += [pscustomobject]@{ Name = "docker-run-demo"; Type = "mcp_server"; Grade = "B"; Client = "claude-code"; Desc = "演示 docker_run 安装方式：拉取 nginx:alpine 镜像并生成运行配置；nginx 不是 MCP server，本包不会注册到客户端 MCP。"; Method = "docker_run"; ZipName = $null; Sha256 = $null; Size = $null; RootName = $null; McpServers = $null; Compat = $null; Targets = $null }
